...
- [Release notes 8.4.3](gmc_radiation_monitor/RELEASE_NOTES_8.4.3.md)
...
## Version 8.4.3 highlights

- Fixes the 8.4.2 startup migration error `jq: syntax error, unexpected IDENT`.
- Persists the complete GMC-320/GMC-500+ configuration through one structured Supervisor API JSON request.
- Preserves arrays, quoted calibration references, plus signs, percentages and Unicode units such as `µSv/h` without shell interpolation.
- Keeps the 8.4.2 single-/dual-tube configuration cleanup and all measurement algorithms unchanged.
- Falls back to the existing lossless runtime migration if the Supervisor cannot persist options.
- Updates all eight bundled manuals and adds regression tests for the exact failing configuration shape.
