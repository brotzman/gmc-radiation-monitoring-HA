...
Version 8.4.2 intentionally has no `image:` entry in the app configuration. If Home Assistant still tries to pull `ghcr.io/brotzman/gmc-radiation-monitor`, refresh the app repository metadata and verify that GitHub contains the 8.4.2 commit. Removing and re-adding the custom repository can clear stale metadata; existing app data should be backed up before uninstalling an installed instance.
...
- [Release notes 8.4.2](gmc_radiation_monitor/RELEASE_NOTES_8.4.2.md)
...
## Version 8.4.2 highlights

- Removed duplicate measurement-quality and device-comparison tiles from the dashboard.
- Improved Detector profile and Expert-view spacing so labels and values no longer wrap letter by letter.
- GMC-320 remains a single-tube configuration without advanced dual-tube fields.
- GMC-500+ is configured once as a complete M4011 + SI-3BG device; legacy split options are migrated losslessly.
- Predefined values are identified as application working values rather than certified factory calibration.
- Public defaults no longer contain installation-specific serial numbers or orientation calibration values.
- All eight interface/configuration translations and bundled manuals were updated.
