...
Version 10.0.20 reduces the width, spacing and height of the view selector, language selector, connection-status badge, section selector and refresh button on desktop. This gives the title area enough room to keep the German subtitle on one line. The controls remain at the top right, the section selector remains directly below the status badge, and only that selector floats while scrolling. Mobile layouts retain their existing reflow behavior.
...
Version 10.0.15 removes the **Measurement display / Main measurement font size** card, the corresponding Home Assistant app options and the browser-local override. The primary live measurement always uses the former **medium** size for a consistent layout on all clients.

The German dashboard and browser title was shortened from **GMC-Strahlungsüberwachung** to **Strahlungsüberwachung**. The subtitle **Lokale Überwachung für GQ-GMC-Geigerzähler** remains unchanged. Database schema 8, stored measurements, MQTT identities, detector profiles, thresholds, scientific calculations and serial protocols remain unchanged.
...
### Connected-device card and live-value sizing (8.4.6)
...
The main live measurement now uses a fixed medium font size. The former add-on options and browser-local **Measurement display** control have been removed so the layout remains consistent on every device.
...
The former browser-local main-value sizing override has been removed in version 10.0.15. The detector Expert view now uses the same top-right disclosure control and full-width layout as the other expandable cards. Public defaults no longer ship installation-specific temperature or weather entity IDs; pressure-based hints remain disabled until entities are configured.
