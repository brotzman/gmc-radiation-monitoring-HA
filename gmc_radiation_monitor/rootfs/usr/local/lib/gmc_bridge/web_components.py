...
def render_analysis_level_controls(*, default_level: str, translator: TranslatorLike) -> str:
    level = AnalysisLevel.normalize(default_level)
    labels = (
    ...
    buttons = "".join(
        ...
        f'aria-controls="view-overview view-analysis view-long-term view-history view-workflow view-calibration view-reports view-maintenance" '
    ...
    return (
        ...
        f'<span>{html.escape(translator("Choose how many details and tools are shown across all sections. The selection is stored in this browser."))}</span></div>'
