...
class WorkflowApplicationMixin:
    ...
    def api_config_validation(self) -> dict[str, Any]:
        try:
            payload = json.loads(Path("/data/options.json").read_text(encoding="utf-8"))
            problems = validate_candidate_options(payload)
            source = "/data/options.json"
        except (OSError, ValueError, json.JSONDecodeError) as exc:
            problems = [str(exc)]
            source = "unavailable"
        return {"valid": not problems, "source": source, "problems": problems}
