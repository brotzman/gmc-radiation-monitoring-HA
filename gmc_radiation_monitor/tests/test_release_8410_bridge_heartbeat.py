...
def test_8410_bridge_heartbeat_reporter_writes_atomic_liveness_record(tmp_path: Path) -> None:
...
def test_8410_automatic_bridge_reports_runtime_state_transitions() -> None:
    class FakeDiscovery:
...
def test_8410_health_distinguishes_bridge_liveness_from_measurement_freshness(tmp_path: Path) -> None:
    now = int(time.time())
    system = {
...
def test_8410_health_limits_are_validated_and_exported_to_bridge(tmp_path: Path) -> None:
...
def test_8410_release_metadata_and_public_defaults() -> None:
    ...
    assert (ROOT / "RELEASE_NOTES_9.1.0.md").is_file()
