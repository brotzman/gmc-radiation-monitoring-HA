...
<header class="report-header">
  <div class="header-topline">
    ...
    <div class="header-tools"><div class="analysis-select-group"><label class="analysis-select-label"><span class="visually-hidden">Ansicht</span><span class="analysis-select-control"><select id="analysis-level-select" aria-label="Ansicht"><option>◉ Zusammenfassung</option><option selected>▤ Analyse</option><option>∑ Experte</option></select></span></label><small class="header-select-description">Detaillierte Interpretation und die wichtigsten unterstützenden Werte</small></div><form class="language-select-form"><label class="language-select-label"><span class="visually-hidden">Sprache</span><span class="language-select-control"><span class="language-select-icon" aria-hidden="true">🌐</span><select id="language-select" aria-label="Sprache"><option>Automatisch</option><option selected>Deutsch</option><option>English</option></select></span></label></form><div class="header-status-column"><a class="header-status-badge warning"><span class="header-status-symbol">🟡</span><span class="header-status-text">1/2 verbunden</span></a><div class="section-navigation-slot" id="section-navigation-slot"><div class="section-floating-navigation" id="section-floating-navigation"><label class="section-select-label"><span class="visually-hidden">Bereich</span><span class="section-select-control"><select id="section-select" aria-label="Bereich"><optgroup label="Übersicht"><option>▣ Geräte</option><option>✦ Intelligente Auswertung</option></optgroup><optgroup label="Auswertung"><option selected>▤ Analyse</option><option>↗ Langzeit</option><option>⌁ Verlauf</option></optgroup></select></span></label></div></div></div><button class="header-tool-button reload-button" id="reload-dashboard" aria-label="Seite aktualisieren" title="Seite aktualisieren"><span class="header-tool-icon">↻</span><span class="header-tool-label visually-hidden">Aktualisieren</span></button></div>
  </div>
</header>
            ...
            for width in VIEWPORT_WIDTHS:
                ...
                result = page.evaluate(
                    """async () => {
                      ...
                      const selectors = ['main','header','section','.content-utility-bar','.header-topline','.header-tools','.analysis-select-group','.analysis-select-label','.language-select-form','.language-select-label','.header-status-column','.header-status-badge','.section-navigation-slot','.section-floating-navigation','.section-select-label','.device-card','.table-wrap','.long-term-table-wrap','.calendar-scroll','.calibration-wizard'];
                      const outside = [];
